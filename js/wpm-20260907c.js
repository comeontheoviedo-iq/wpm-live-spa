...
function chrome(inner, title){
  ...
  <nav class="tabbar">
    ...
    <a class="${path()==="/rankings"?"on":""}" href="/rankings">Rankings</a>
...
function viewRankings(){
  ...
  const list = rows.slice(0, 60).map(r => `
    <div class="match" style="grid-template-columns:48px 1fr auto">
      <div class="st">${r.rank}</div>
      <div>
        <div class="a">${r.name}</div>
        <div class="games">${r.country||""}${r.dupr?" · DUPR "+r.dupr:""}</div>
      </div>
      <div class="score">${r.points!=null?r.points:(r.elo||"")}</div>
    </div>`).join("");
