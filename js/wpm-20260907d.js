...
function weekStrip(){
  ...
  return `<div class="panel"><div class="kicker">This week on the GPA calendar</div>${soon.map(e=>`<div class="rank-row"><b></b><div><strong>${e.name||''}</strong><span>${(e.tournament_date||'').slice(0,10)} · ${e.location||e.venue||''} · ${e.tier||''}</span></div><em>${e.host||''}</em></div>`).join('')}<a class="chip" href="/rankings">Full table</a></div>`;
}
...
function viewRankings(){
  ...
  const board = state.rankBoard || "gpa";
  const cats = [
  ...
  if (board === "gpa") {
    ...
    note = "GPA world rankings · rolling 12 months · best 10 results · gpapickleball.org";
  } else {
    rows = (cat.indexOf("double")>=0 ? data.elo && data.elo.mensDoubles : data.elo && data.elo.singles) || [];
    note = "Pro ELO from public PickleWave boards · PPA + APP + MLP match pool. Restyled for WPM.";
  }
  ...
  return `<div class="hero"><h2>RANKINGS</h2><p>GPA world list. PPA is a different number. ELO is form across tours.</p></div>
  <div class="wrap">
    <div class="panel">
      <div class="seg">
