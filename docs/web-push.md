...
**Slice:** 2026-09-18 · client `wpm-20260918b.js` · SW `sw.js?v=20260918b`  
...
| **`push-live-check`** | Scheduled every 5 min (`*/5 * * * *`). Fetches `/api/ppa` + `/api/app` + `/api/worldcup`, finds LIVE rows, matches follow tags, sends via `web-push`. |
...
- Match if any `m.tags` ∈ subscription `follows`.
- De-dupe: `notified[matchId] = timestamp`; skip if set; TTL ~6h.
- 404/410 from push service → delete subscription.

## How to verify

1. Hard refresh https://live.worldpickleballmagazine.com — JS `wpm-20260918b.js`, SW `20260918b`.
...
- Follow tags on boards are still sparse (seed names like Waters/Johns/Bright + whatever ingest adds). Untagged LIVE matches never fire.
