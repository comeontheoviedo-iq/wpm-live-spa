...
function lookupPerson(kind, id){
  ...
  if (kind!=="player") {
    const mlp=(((state.history||{}).mlp||{}).standings||[]).find(t => (t.team||"").toLowerCase()===id.toLowerCase());
    if (mlp) return {name:mlp.team, role:`MLP 2026 · #${mlp.rank} · ${mlp.pts} pts`, blurb:"2026 Major League Pickleball regular season."};
    return TEAMS[id] || {name:id, role:"Team", blurb:"Team page from archive ties."};
  }
